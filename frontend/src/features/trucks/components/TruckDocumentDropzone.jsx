...
import { useUploadDocument, useDeleteDocument } from '../hooks/useTruckDocuments';

export const TruckDocumentDropzone = ({ truckId, documentType, existingDocument }) => {
  const { mutate: uploadDoc, isPending: isUploading } = useUploadDocument();
  const { mutate: deleteDoc, isPending: isDeleting } = useDeleteDocument();

  const onDrop = useCallback((acceptedFiles) => {
    if (acceptedFiles?.length) {
      uploadDoc({ truckId, documentType, file: acceptedFiles[0] });
    }
  }, [truckId, documentType, uploadDoc]);
  ...
  if (existingDocument) {
    return (
      <div className="p-4 border rounded-xl flex items-center justify-between bg-card">
        ...
        <button 
          onClick={() => deleteDoc({ truckId, documentId: existingDocument.id })}
          disabled={isDeleting}
